...
$requirements = Join-Path ([IO.Path]::GetTempPath()) 'cookfully-audit-requirements.txt'
...
try {
    ...
    Invoke-Checked 'SC-008 independent-agent proxy evidence' { uv run --directory backend cookfully usability-study validate-proxy --input ../artifacts/usability-proxy-data.json --output ../artifacts/usability-proxy-summary.json --require-pass }
    ...
    Invoke-Checked '50-recipe nutrition corpus' { uv run --directory backend cookfully nutrition-corpus run --require-pass --output ../artifacts/nutrition-release-report.json }
    ...
    $developmentCookieSecure = $env:COOKFULLY_COOKIE_SECURE
    $developmentPublicBaseUrl = $env:COOKFULLY_PUBLIC_BASE_URL
    $developmentApiBaseUrl = $env:COOKFULLY_API_BASE_URL
    $developmentTrustedProxies = $env:COOKFULLY_TRUSTED_PROXY_CIDRS
    try {
        $env:COOKFULLY_COOKIE_SECURE = 'true'
        $env:COOKFULLY_PUBLIC_BASE_URL = 'https://planner.example.test'
        $env:COOKFULLY_API_BASE_URL = 'https://planner.example.test'
        $env:COOKFULLY_TRUSTED_PROXY_CIDRS = '172.31.250.10/32'
        ...
        $env:COOKFULLY_COOKIE_SECURE = $developmentCookieSecure
        $env:COOKFULLY_PUBLIC_BASE_URL = $developmentPublicBaseUrl
        $env:COOKFULLY_API_BASE_URL = $developmentApiBaseUrl
        $env:COOKFULLY_TRUSTED_PROXY_CIDRS = $developmentTrustedProxies
    }
    ...
    $env:COOKFULLY_PERFORMANCE_REPORT_CONTAINER = '/app/artifacts/performance-release-report.json'

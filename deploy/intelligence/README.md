...
Place the pinned Needle 2 model artifact at `needle2.cact` in the
`intelligence-model-data` volume before enabling production inference. The
artifact must be obtained from the approved model release and verified against
the release checksum. Do not mount the application media or database volumes
into this service.

...
**Tests**: Tests are REQUIRED when the feature affects nutrition values, goal totals, grocery
aggregation, API/MCP contracts, background jobs, external-service boundaries, or a critical user
journey. For other work, include tests when requested by the feature specification or plan.
...
- [ ] T010 Define API contracts and reusable application-service boundaries
- [ ] T011 [P] Configure background-job retries, idempotency, and observable failure states if needed
- [ ] T012 [P] Add nutrition provenance, estimation status, and manual-override precedence if needed
...
### Tests for User Story 1 (REQUIRED when mandated by the constitution or plan) ⚠️
...
### Tests for User Story 2 (REQUIRED when mandated by the constitution or plan) ⚠️
...
### Tests for User Story 3 (REQUIRED when mandated by the constitution or plan) ⚠️
...
- [ ] TXXX Validate loading, empty, partial, estimated, and failed states
- [ ] TXXX Verify keyboard access, contrast, overflow, desktop, and narrow-mobile layouts
- [ ] TXXX Update API/MCP contracts and self-hosting, backup, and export guidance if affected
- [ ] TXXX Record dependency reuse decisions, licenses, and pinned versions

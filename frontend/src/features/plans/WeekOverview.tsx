...
function DraggableWeekMeal({ entry, recipe, onCopy, onDelete, pending }: { entry: MealPlanEntry; recipe?: Recipe; onCopy: () => void; onDelete: () => void; pending: boolean }) {
  const { ref, isDragging } = useDraggable<MealDragData>({ id: entry.id, data: { kind: "meal", entry }, disabled: !entry.recipeId || pending });
  return (
    <article ref={ref} className={`week-meal${isDragging ? " is-dragging" : ""}`}>
      {entry.recipeId ? <Link className="week-meal__body" to={`/app/recipes/${entry.recipeId}`} aria-label={`Open ${entry.recipeTitle}`}><span className="week-meal__media">{recipe ? <RecipeMedia recipe={recipe} /> : <RecipeFallbackArt title={entry.recipeTitle} />}</span><div className="week-meal__copy"><strong>{entry.recipeTitle}</strong><span>{formatCookingNumber(entry.servings)} {Number(entry.servings) === 1 ? "serving" : "servings"}</span>{recipe ? <RecipeMetadata recipe={recipe} compact /> : null}</div></Link> : <div className="week-meal__body"><span className="week-meal__media"><RecipeFallbackArt title={entry.recipeTitle} /></span><div className="week-meal__copy"><strong>{entry.recipeTitle}</strong><span>{formatCookingNumber(entry.servings)} {Number(entry.servings) === 1 ? "serving" : "servings"}</span></div></div>}
      <div className="week-meal__actions">
        <button type="button" className="week-meal__copy-action" disabled={!entry.recipeId || pending} onPointerDownCapture={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()} onClick={onCopy} aria-label={`Copy ${entry.recipeTitle}`} title="Copy meal"><Copy aria-hidden="true" /></button>
        <button type="button" className="week-meal__delete" disabled={pending} onPointerDownCapture={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()} onClick={onDelete} aria-label={`Delete ${entry.recipeTitle}`} title="Delete meal"><X aria-hidden="true" /></button>
      </div>
    </article>
  );
}

function WeekSlot({ date, slot, entries, recipesById, onAdd, copyPlacement, pending, onPlace, onStartCopy, onDelete }: { date: string; slot: string; entries: MealPlanEntry[]; recipesById: Map<string, Recipe>; onAdd: () => void; copyPlacement: CopyPlacement | null; pending: boolean; onPlace: (date: string, slot: string) => void; onStartCopy: (entry: MealPlanEntry) => void; onDelete: (entry: MealPlanEntry) => void }) {
  ...
  return (
    <section ref={ref} className={`week-slot${isDropTarget ? " is-drop-target" : ""}${copyPlacement ? " week-slot--placement-target" : ""}`} aria-label={`${label} on ${weekday(date)}${copyPlacement ? ` — ${placementLabel}` : ""}`}>
      <header><span>{label}</span>{entries.length ? <small>{entries.length}</small> : null}</header>
      {entries.map((entry) => <DraggableWeekMeal key={entry.id} entry={entry} recipe={entry.recipeId ? recipesById.get(entry.recipeId) : undefined} pending={pending} onCopy={() => onStartCopy(entry)} onDelete={() => onDelete(entry)} />)}
          ...
          {dates.map((date) => {
            ...
            <article className={`week-day${date === selectedDate ? " week-day--selected" : ""}`} key={date}>
              <button className="week-day__header" onClick={() => onOpenDay(date)} aria-label={`Open day view for ${weekday(date)}, ${longDate(date)}`}>
                ...
                <small>{dayEntries.length ? `${dayEntries.length} ${dayEntries.length === 1 ? "meal" : "meals"}` : "Open"}</small>
              </button>

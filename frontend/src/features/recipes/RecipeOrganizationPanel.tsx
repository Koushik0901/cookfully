...
export function RecipeOrganizationPanel({ recipe, onSaved }: { recipe: RecipeDetail; onSaved: (value: RecipeDetail) => void }) {
  ...
  const save = useMutation({ mutationFn: (value: { favorite: boolean; collectionIds: string[]; mealRoles: (typeof roles)[number][] }) => recipesApi.organize(recipe.id, recipe.version, value), onSuccess: (value) => { onSaved(value); void queryClient.invalidateQueries({ queryKey: ["recipes"] }); } });
  ...
  const organizationValue = () => ({ favorite, collectionIds: selected, mealRoles: mealRoles as (typeof roles)[number][] });
  const toggleFavorite = () => {
    const next = !favorite;
    setFavorite(next);
    save.mutate({ ...organizationValue(), favorite: next }, { onError: () => setFavorite(!next) });
  };
  return <>
    <button type="button" className={`recipe-organization__quick-favorite${favorite ? " is-favorite" : ""}`} aria-pressed={favorite} onClick={toggleFavorite} disabled={save.isPending}>
      <Heart aria-hidden="true" />{favorite ? "Saved to favorites" : "Add to favorites"}
    </button>

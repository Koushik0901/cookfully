...
function CollectionRow({ collection, first, last }: { collection: RecipeCollection; first: boolean; last: boolean }) {
  ...
  return <li className="collection-manager__row"><input className="input" aria-label={`${collection.name} collection name`} value={name} onChange={(event) => setName(event.currentTarget.value)} /><Button className="button--icon" aria-label={`Save name for collection ${collection.name}`} disabled={!name.trim() || name === collection.name || update.isPending} onClick={() => update.mutate({ name })}><Check aria-hidden="true" /></Button><Button className="button--icon" aria-label={`Move ${collection.name} earlier`} disabled={first || update.isPending} onClick={() => update.mutate({ position: collection.position - 1 })}><ArrowUp aria-hidden="true" /></Button><Button className="button--icon" aria-label={`Move ${collection.name} later`} disabled={last || update.isPending} onClick={() => update.mutate({ position: collection.position + 1 })}><ArrowDown aria-hidden="true" /></Button><Button className="button--icon" aria-label={`Delete ${collection.name} collection`} disabled={remove.isPending} onClick={() => remove.mutate()}><Trash2 aria-hidden="true" /></Button>{update.error instanceof Error || remove.error instanceof Error ? <p className="error-text" role="alert">{(update.error ?? remove.error as Error).message}</p> : null}</li>;
}

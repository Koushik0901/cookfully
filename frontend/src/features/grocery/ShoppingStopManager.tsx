...
function StopRow({ stop, first, last }: { stop: GroceryShoppingStop; first: boolean; last: boolean }) {
  ...
  return <li className="shopping-stops__row"><input className="input" aria-label={`${stop.name} stop name`} value={name} onChange={(event) => setName(event.currentTarget.value)} /><Button className="button--icon" aria-label={`Save name for stop ${stop.name}`} disabled={!name.trim() || name === stop.name || update.isPending} onClick={() => update.mutate({ name })}><Check aria-hidden="true" /></Button><Button className="button--icon" aria-label={`Move ${stop.name} earlier`} disabled={first || update.isPending} onClick={() => update.mutate({ position: stop.position - 1 })}><ArrowUp aria-hidden="true" /></Button><Button className="button--icon" aria-label={`Move ${stop.name} later`} disabled={last || update.isPending} onClick={() => update.mutate({ position: stop.position + 1 })}><ArrowDown aria-hidden="true" /></Button><Button className="button--icon" aria-label={`Remove ${stop.name}`} onClick={() => remove.mutate()} disabled={remove.isPending}><Trash2 aria-hidden="true" /></Button>{error instanceof Error ? <p className="error-text" role="alert">{error.message}</p> : null}</li>;
}

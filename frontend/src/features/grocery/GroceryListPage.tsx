  ...
  return (
    <article className={`grocery-item ${checked ? "grocery-item--checked" : ""}`}>
      <div className="grocery-item__heading">
        ...
        <div><h3>{title}</h3><p className="data-value">{item.quantity ? formatCookingNumber(item.quantity) : "As needed"}{item.unit ? ` ${item.unit}` : ""}</p></div>
      ...
      {!readOnly ? <details className="disclosure"><summary>Edit {title}</summary><form className="grocery-edit" onSubmit={save}><Field label={`${title} name`}><input className="input" value={name} onChange={(event) => setName(event.target.value)} /></Field><Field label={`${title} quantity`}><DecimalInput value={quantity} onInput={(event) => setQuantity(event.currentTarget.value)} /></Field><Field label={`${title} unit`}><input className="input" value={unit} onChange={(event) => setUnit(event.target.value)} /></Field><Field label={`Shopping stop for ${title}`}><Select value={item.shoppingStop?.id ?? ""} onChange={(event) => update.mutate({ shoppingStopId: event.target.value || null })}><option value="">Unassigned</option>{stops.map((stop) => <option value={stop.id} key={stop.id}>{stop.name}</option>)}</Select></Field>{item.origin === "generated" && !item.needsReview && item.shoppingStop ? <label className="check-label"><Checkbox checked={rememberPlacement} onCheckedChange={(value) => setRememberPlacement(value === true)} />Remember this stop for {title}</label> : null}<Button variant="secondary" type="submit" disabled={!name.trim() || update.isPending}>Save {title}</Button>{rememberPlacement ? <Button variant="ghost" type="button" onClick={() => update.mutate({ rememberPlacement: true })}>Remember placement</Button> : null}</form></details> : null}
      {item.sources.length ? <div><Button variant="ghost" aria-expanded={showSources} onClick={() => setShowSources((value) => !value)}>{showSources ? "Hide" : "Show"} {title} sources</Button>{showSources ? <ul className="source-list">{item.sources.map((source) => <li key={`${source.mealPlanEntryId}-${source.originalText}`}><span>{source.originalText}</span><span className="data-value">{source.quantityContribution ? formatCookingNumber(source.quantityContribution) : "Unquantified"}{item.unit && source.quantityContribution ? ` ${item.unit}` : ""}</span></li>)}</ul> : null}</div> : <p className="muted">Manual item</p>}
      ...
      {!readOnly ? <div className="actions">{conflict ? <Button onClick={() => void refresh()}>Reload list</Button> : null}<Button variant="ghost" onClick={() => remove.mutate()} disabled={remove.isPending}>Remove {title}</Button></div> : null}
    </article>
  );
}
...
export function GroceryListPage() {
  ...
  if (missing || !list.data) return <main className="page-shell grocery-page"><EmptyState headingLevel="h1" title="Your grocery list starts with your plan" description="Add meals to this week, then Cookfully will gather the ingredients and quantities here." action={<><Button asChild><Link to="/app/plan">Open meal plan</Link></Button><Button variant="secondary" onClick={() => regenerate.mutate()} disabled={regenerate.isPending}>{regenerate.isPending ? "Starting your list…" : "Start an empty list"}</Button></>} /></main>;

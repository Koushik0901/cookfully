...
export function GoalSettingsPage() {
  ...
  return (
    <main className="page-shell goals-page">
      <PageHeader eyebrow="Your nutrition guide" title="Shape how Cookfully plans for you" description="Set a complete daily guide for plans. It shapes meals and weeks—not a pass/fail score for individual meals." actions={<Button variant="secondary" asChild><Link to="/app/plan">Back to meal plan</Link></Button>} />
      <form className="goal-form" onSubmit={submit} noValidate>
        ...
        <section className="goal-targets" aria-labelledby="daily-guide-title">
          <div className="goal-section-heading"><div><p className="eyebrow">Then, the daily guide</p><h2 id="daily-guide-title">Nutrition to plan around</h2></div><div className="goal-targets__heading-side"><p>Calories, protein, carbohydrate, and fat are required to balance a day and week. Fiber and sodium are optional.</p>{!showTargetInputs ? <Button type="button" variant="secondary" onClick={() => { setSaved(false); setEditingTargets(true); }}><PencilLine aria-hidden="true" />Adjust daily guide</Button> : null}</div></div>
          {showTargetInputs ? <div className="target-grid">
            <div className="target-field target-field--calories"><span className="target-field__icon"><Gauge aria-hidden="true" /></span><Field label="Daily calories (required)" error={errors.caloriesKcal} accessibilityLabel="Daily calories"><DecimalInput value={caloriesKcal} onInput={(event) => onCaloriesChange(event.currentTarget.value)} /></Field><span className="target-field__unit">kcal</span></div>
            <div className="target-field target-field--protein"><span className="target-field__dot" aria-hidden="true" /><Field label="Daily protein (required)" error={errors.proteinG} accessibilityLabel="Daily protein"><DecimalInput value={proteinG} onInput={(event) => { markChanged(); setProteinG(event.currentTarget.value); }} /></Field><span className="target-field__unit">grams</span></div>
            <div className="target-field target-field--carbs"><span className="target-field__dot" aria-hidden="true" /><Field label="Daily carbohydrate (required)" error={errors.carbohydrateG} accessibilityLabel="Daily carbohydrate"><DecimalInput value={carbohydrateG} onInput={(event) => { markChanged(); setCarbohydrateG(event.currentTarget.value); }} /></Field><span className="target-field__unit">grams</span></div>
            <div className="target-field target-field--fat"><span className="target-field__dot" aria-hidden="true" /><Field label="Daily fat (required)" error={errors.fatG} accessibilityLabel="Daily fat"><DecimalInput value={fatG} onInput={(event) => { markChanged(); setFatG(event.currentTarget.value); }} /></Field><span className="target-field__unit">grams</span></div>

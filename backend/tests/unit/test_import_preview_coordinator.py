...
def owned_coordinator(session_factory, tmp_path):
    ...
    class StubRecipes:
        def create(self, write, *, trace_id: str, owner_id: UUID):
            created.append((write, trace_id, owner_id))
            return SimpleNamespace(
                recipe=SimpleNamespace(id=uuid7(), title=write.title),
...
async def test_confirm_expired_or_missing_preview_raises_410(
    ...
    with pytest.raises(DomainError) as missing:
        owned_coordinator.confirm("nope", {}, owner_id=owner_id, trace_id="t")
    ...
    with pytest.raises(DomainError) as expired:
        owned_coordinator.confirm(parse_id, {}, owner_id=owner_id, trace_id="t")
...
async def test_confirm_applies_title_yield_and_quantity_overrides_and_persists(
    ...
    write, _trace_id, owner_arg = owned_coordinator.created[-1]

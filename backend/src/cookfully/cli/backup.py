...
from cookfully.application.exports import _json_value, _ndjson, _row, _safe_member
    ...
    try:
        with zipfile.ZipFile(archive) as bundle:
            ...
            if manifest.get("schemaVersion") != 1 or manifest.get("kind") != (
                "cookfully-disaster-recovery-backup"
            ):
                raise DomainError("backup_manifest_invalid", "Backup manifest is invalid.", 422)
...
class BackupManager:
    ...
    def create(
        ...
        *,
        ...
        manifest = {
            "schemaVersion": 1,
            "kind": "cookfully-disaster-recovery-backup",
...
def create_command(
    ...
    try:
        ...
        timestamp = utc_now()
        target = output / f"cookfully-backup-{timestamp:%Y%m%dT%H%M%SZ}.zip"
        manager = BackupManager(
            sessions,
            MediaStore(settings.media_root, settings.secret_key.get_secret_value()),
            ErasureLedger(settings.erasure_ledger_root),

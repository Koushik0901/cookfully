...
def resolve_expiry(
    display_name: str,
    requested_expires_on: date | None = None,
    today: date | None = None,
) -> tuple[date | None, str | None, datetime | None, bool]:
    now = utc_now()
    cur_today = today or now.date()
    if requested_expires_on is not None:
        # caller will decide label vs manual; first prompt = label,
        # later edits = manual (service decides)

...
from vigor_vine.application.access_tokens import AccessTokenPrincipal, AccessTokenService
from vigor_vine.domain.common import DomainError
from vigor_vine.infrastructure.observability import correlation_id, safe_log
...
class RedisTokenRateLimiter:
    ...
    def enforce(self, token_id: str, category: RateCategory) -> None:
        key = f"vv:mcp:rate:{token_id}:{category}"
        try:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            response = JSONResponse(
                {
                    "type": f"urn:vigor-vine:problem:{exc.code}",

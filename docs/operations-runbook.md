...
4. Monitor 24h: `needle_inline_applied` rate, `p95 latency <600ms`, and
   `peak_ram_mb <64MB` per request with the bundled Needle2 runtime. The
   reported value includes the model process overhead (a healthy local check
   is typically in the high-50 MB range), so do not compare it with the old
   lightweight synthetic benchmark. Watch for a `skipped_invalid` rise; logs
   contain `confidence/reasoning` but never user text. Breach → bump `T` +0.05
   or set `INLINE_ENABLED=false` (no-op rollback).

...
**Design:**
- Determine new file at implementation: `git diff HEAD -- frontend/public/brand/` + `git log --oneline -10 -- public/brand App.tsx` to identify which `brand/*.svg` changed for the rail. If `cookfully-mark.svg` is the refreshed file, keep `BrandMark` `src="/brand/cookfully-mark.svg"` (overwrite with new SVG). If a new `cookfully-logo.svg` wordmark was added for the rail, switch `BrandMark` to `/brand/cookfully-logo.svg` for shell/auth screens and keep `cookfully-mark.svg` as the square favicon source. Log chosen file in commit message.

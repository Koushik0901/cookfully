...
pypdf's image embedding is awkward for fixtures, so extract images from a small PDF that is generated by writing an image+content-stream by hand. Use a helper that builds a 1-page PDF with one embedded JPEG and asserts the importer surfaces it. Add `backend/tests/unit/test_recipe_importer.py` (create if missing):
...
def _single_image_pdf() -> bytes:
    """Build a 1-page PDF with one embedded 8x8 JPEG using pypdf primitives."""
    import pypdf
    ...
    obj = writer._add_object(StreamObject())
    obj.set_data(jpg.getvalue())
    obj[NameObject("/Type")] = NameObject("/XObject")
    obj[NameObject("/Subtype")] = NameObject("/Image")
    obj[NameObject("/Width")] = NumberObject(8)
    obj[NameObject("/Height")] = NumberObject(8)
    ...
    resources = DictionaryObject({NameObject("/XObject"): DictionaryObject({NameObject("/Im0"): obj})})
...
def test_pdf_image_extraction_via_pypdf():
    """pypdf can read embedded raster images; the importer maps them to candidates."""
    content = _single_image_pdf()
    urls = RecipeImporter._pdf_image_candidates(content)
    assert len(urls) == 1
```
The test calls only the public helper `_pdf_image_candidates(content)` from Step 4 (and the fixture `_single_image_pdf` above), so it compiles once the implementation lands.
...
@staticmethod
def _pdf_image_candidates(content: bytes) -> tuple[str, ...]:
    """Return opaque descriptors for raster images embedded in a cookbook PDF.

    pypdf exposes ``page.images`` (each with ``.data`` bytes and ``.image``
    PIL object). We return these as ``pdf:{index}`` placeholders so the
    preview/confirm flow can offer a thumbnail choice without persisting
    anything yet. The confirm step (Task 4) persists the chosen image via the
    existing media capture path.
    """
    reader = PdfReader(io.BytesIO(content), strict=False)
    urls: list[str] = []
    for page in reader.pages:
        for image in getattr(page, "images", ()):
            _ = image.data  # validate bytes are present
            urls.append(f"pdf:{len(urls)}")
    return tuple(urls)
```
Wire `content` into `_import_pdf` (it already receives `content: bytes`) and pass the per-recipe candidates so `image_candidates` is non-empty whenever the pages embed images. Mirror the existing "no silent choice with >1 image" rule.

- [ ] **Step 5: Run tests until green** — expect PASS

- [ ] **Step 6: Commit**

```bash
git add backend/src/cookfully/infrastructure/recipe_importer.py backend/tests/unit/test_recipe_importer.py
git commit -m "feat: expose image candidates and PDF images for import preview"
```
